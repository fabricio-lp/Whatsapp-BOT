...
let phoneNumber = "5199999999"; // alterar número
...
// Função para formatar números com ponto de milhar (ex: 1000 -> 1.000)
const formatarTexto = (texto) => {
  if (typeof texto !== "string") return texto;
  // Formata sequências de 4 ou mais dígitos que não sejam menções, decimais ou datas
  return texto.replace(/(?<![@\d.,\/-])\d{4,}(?![.,\d\/-])/g, (match) => {
    return match.replace(/\B(?=(\d{3})+(?!\d))/g, ".");
  });
};

// Função para remover pontos de milhar e converter para número puro (ex: 100.000 -> 100000)
...
// Variável global para o intervalo da lootbox
...
// Funções auxiliares da Lootbox
function getLootboxState() {
  if (!fs.existsSync(lootboxFile)) {
    // Se não existe, cria com o próximo drop para daqui a 6 horas
...
async function startProo() {
  ...
  const sock = makeWASocket({
    ...
    printQRInTerminal: false, // Desativado para não mostrar QR
  ...
  // Sobrescreve o sendMessage para formatar números automaticamente em todas as mensagens enviadas
  ...
  // 🟢 Se não houver sessão registrada, gerar o código de vinculação de 8 dígitos
  ...
  //SISTEMA DE LOOTBOX AUTOMÁTICO
  if (lootboxInterval) clearInterval(lootboxInterval); // Limpa intervalo anterior se houver reconexão
  ...
  lootboxInterval = setInterval(async () => {
    try {
      ...
      // Verifica se não está ativa e se já passou do horário do drop
      if (!lbState.active && now >= lbState.nextDrop) {
        // Define o prêmio entre 10.000 e 500.000
    ...
    try {
      ...
      for (let num of participants) {
        ...
        if (anu.action == "add") {
          ...
          const sol = ``; //mensagem de boas-vindas personalizada
  ...
  sock.ev.on("messages.upsert", async (m) => {
    console.log("Mensagem recebida de:", m.messages[0]?.key?.remoteJid); // DEBUG LOG
    try {
      ...
      // 🟢 Sistema de ligar/desligar global do bot
      ...
      // Função para validar se um valor é um inteiro válido (não NaN, não decimal, não string)
      ...
      const enviar = (texto) => {
        try {
          sock.sendMessage(from, { text: texto }); // Temporariamente removido { quoted: info }
        } catch (e) {
...
▰▰ ${Mrxp} ▰▰`;
        ...
        case "tigrinho":
          ...
          if (tempoRestante > 0) {
            ...
            const espera = 15 * 1000; // 15 seg
            await adicionarCooldownCacaNiqueis(sender, espera);
          }

          // Reduzir 1 moeda e adicionar 1 de experiência
          await removerMoedas(sender, apostas);

          // Lista de símbolos para o caça-níqueis
          const simbolos = [
            "🥕",
            "🐰",
            "🐸",
          ...
          if (probabilidad < 0.65) {
            const simboloVencedor =
              simbolos[Math.floor(Math.random() * simbolos.length)];
            filaCentro = [simboloVencedor, simboloVencedor, simboloVencedor]; // Fazer com que os 3 sejam iguais
          } else {
            filaCentro = obterLinha(); // Se não, gerar aleatoriamente
          }
          ...
          if (vencedor) {
            ...
            const tipoPremio = Math.random() < 0.8 ? "coins" : "exp"; // 50% de probabilidade para cada tipo
...
${premioTexto}`;
          ...
          // Enviar a mensagem após 3 segundos
          setTimeout(() => {
        ...
        case "levelup":
          {
            ...
            // Cálculo direto para evitar travamentos com valores de XP muito altos
            if (XpR >= Rxxp + 20) {
            ...
            if (niveisParaUpar > 0) {
              // Calcula o nível final que o cara vai ficar
              const novoNivel = nivelAtual + niveisParaUpar;

              // Lógica da Patente (Múltiplos de 5)
              // Divide por 5 e arredonda pra baixo. Se o número mudar, ele cruzou uma patente.
              // Ex: 4/5 = 0.8 (0) | 5/5 = 1 (1) -> Mudou! Nova patente.
              let msgPatente = "";
...
${msgPatente}`;
              // A mensagem da patente só aparece se a condição ali em cima for verdadeira
        ...
        case "roleta":
          {
            ...
            if (isMinxxx) {
              ...
              const ppt = ["vivo", "morto"]; // Possibilidades
              const pptb = ppt[Math.floor(Math.random() * ppt.length)]; // Escolha aleatória do sistema
              let vit;
        ...
        case "pix":
          {
            ...
            try {
              const mencionado = obterMencionado(info); // destinatario
              const remetente = sender; // quien envía las monedas
        ...
        case "rankcoins":
          {
            ...
            // Ler os dados atualizados no momento
            ...
            // Converter para array se registro for um objeto
...
Pos.  User   Nível\n`;
        ...
        case "p":
          ...
          try {
            // Realizar a solicitação para a nova API
            ...
            // Verificar se o vídeo é muito longo
        ...
        Se deseja baixar o vídeo, use /playvideo ${q.trim()}`;
            ...
            // Enviar a imagem com as informações do vídeo
            await sock.sendMessage(
              from,
              {
        ...
        case "playmp4":
          {
            try {
              ...
              // Chamada para a nova API do vendedor para buscar o vídeo
        ...
        Se deseja baixar o áudio, use /play ${q.trim()}`;
              ...
              // Enviar informações sobre o vídeo para o usuário
              sock.sendMessage(
                from,
                {
                  image: { url: data[0]?.thumb || logoslink?.logo },
        ...
        case "tiktokvideo":
          try {
            ...
            if (contentType && contentType.includes("application/json")) {
              ...
              // Processa a resposta como um arquivo binário
        ...
        //precisa de api
        case "buscarapk": // Verificação se o usuário é premium
          if (!q.trim()) return enviar(`exemplo: !buscarapk WhatsApp`); // Certificar-se de que haja uma busca

          try {
            enviar("Aguarde..."); // Mensagem de espera
            let abc = await fetchJson(
              `https://api.bronxyshost.com.br/api-bronxys/aptoide_pesquisa?pesquisa=${q.trim()}&apikey=${API_KEY_NAUFRA}`,
            ); // Chamada para a API
            enviar(abc.aptoide || "Nenhuma informação encontrada."); // Resposta com a informação ou mensagem de erro
          } catch (e) {
            console.log(e);
            return enviar(mess.error()); // Mensagem de erro em caso de falha
          }
          break;
        ...
        case "resgatar":
          // Verifica se é o grupo correto (opcional, mas recomendado para segurança)
          try {
            // Grupos permitidos: Oficial e Testes
            const allowedGroups = [
              "120363406690153385@g.us",
              "120363422859824170@g.us",
            ];
            ...
            if (lbState.active) {
              // Usuário conseguiu resgatar!
              lbState.active = false;
              const valorGanho = lbState.amount;

              // Define o próximo drop: Mínimo 6 horas + Tempo aleatório entre 0 e 6 horas
              // Totalizando um intervalo entre 4h e 6h
